package com.lcf.shop.order.action;
...
import com.lcf.shop.cart.vo.Cart;
import com.lcf.shop.cart.vo.CartItem;
...
import com.lcf.shop.utils.PaymentUtil;
...
public class OrderAction extends ActionSupport implements ModelDriven<Order> {
	...
	public String saveOrder() {
		...
		for (CartItem cartItem : cart.getCartItems()) {
			...
			Product p = productService.findByPid(cartItem.getProduct().getPid()) ;
			if(p.getPstock()-cartItem.getCount() < 0)
			{
				this.addActionMessage("商品库存不足");
				return "msg";
			}
			p.setPstock(p.getPstock()-cartItem.getCount());
	...
	public String payOrder() throws IOException {
		...
		String p8_Url = "http://localhost:8080/ssh3_change_bug/order_callBack.action"; // 商户接收支付成功数据的地址:
		String p9_SAF = ""; // 送货地址:
		String pa_MP = ""; // 商户扩展信息:
