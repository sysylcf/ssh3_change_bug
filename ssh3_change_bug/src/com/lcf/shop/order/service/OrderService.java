package com.lcf.shop.order.service;
...
import com.lcf.shop.order.dao.OrderDao;
...
public class OrderService {
	...
	public PageBean<Order> findAllByState(Integer page, int state) {
		PageBean<Order> pageBean = new PageBean<Order>();
		// 设置参数
		pageBean.setPage(page);
		// 设置每页显示的记录数:
		...
		pageBean.setLimit(limit);
		// 设置总记录数
		int totalCount = orderDao.findCountByState(state);
		pageBean.setTotalCount(totalCount);
		// 设置总页数
		int totalPage = 0;
		if(totalCount % limit == 0){
		...
		// 设置每页显示数据集合
		int begin = (page - 1) * limit;
		List<Order> list = orderDao.findByPageByState(begin,limit,state);
		pageBean.setList(list);
		return pageBean;
	}
