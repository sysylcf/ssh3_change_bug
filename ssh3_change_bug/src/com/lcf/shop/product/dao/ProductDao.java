package com.lcf.shop.product.dao;
...
public class ProductDao extends HibernateDaoSupport {
	...
	public int findCountString(String searchString) {
		String hql = "select count(*) from Product p where p.key_word like ?";
		List<Long> list = this.getHibernateTemplate().find(hql, searchString);
		if(list != null && list.size() > 0){
			return list.get(0).intValue();
		}
		return 0;
	...
	public List<Product> findByPageString(String searchString, int begin, int limit) {
		String hql = " from Product  where  key_word like ?";
		List<Product> list = this.getHibernateTemplate().execute(new PageHibernateCallback<Product>(hql, new Object[]{searchString}, begin, limit));
		if(list != null && list.size() > 0){
			return list;
		}
		return null;
